...
`sharding.enabled` 只在启动时选择拓扑。项目不提供配置中心热刷新或运行期切换 API。`sharding.database-sharding-key` 明确表示数据库分片键，必须是 `tenant_id`；它不是季度分表键，后者固定为 `record_date`。
...
```text
Profile: product
Arguments: --sharding.enabled=true --sharding.database-sharding-key=tenant_id --demo.sharding.current-quarter=2026Q3
```
...
- 为什么分库开关只能在部署启动时决定？
- `sharding.database-sharding-key` 为什么在 false 模式下仍作为架构声明保留？
- Q1→q1 的固定映射如何避免轮转错乱？
